...
internal static class IEndpointRouteBuilderExtensions
{
    ...
    {
        routerBuilder.MapGet("/webhooks/clients", async ([FromServices] WebhookClientManager manager) =>
        {
            var clients = await manager.Get();
            return Results.Ok(clients);
        }).WithName("getClients");

        routerBuilder.MapGet("/webhooks/clients/{id}", async ([FromRoute]int id, [FromServices] WebhookClientManager manager) =>
        {
            var client = await manager.Get(id);
            return client.Success ? Results.Ok(client) : Results.BadRequest(client);
        }).WithName("getClient");

        routerBuilder.MapPost("/webhooks/clients/", async ([FromBody] WebhookClient webhookClient, [FromServices] WebhookClientManager manager) =>
        {
